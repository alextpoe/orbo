...
// var Gradient = require('./gradient_draw');
  ...
  this.allObjects().forEach(function (object) {
    if (object.life <= 0){
      if (object instanceof Orb) {
        that.gameOverModal.classList.remove('disappear');
        that.gameOverModalWindow.classList.remove('disappear');
